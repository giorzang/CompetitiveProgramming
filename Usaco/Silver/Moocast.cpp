...
pair<pair<int, int>, int> points[MAXN];
bool visited[MAXN];

int n;

double dis(int a, int b) {
    double x1 = points[a].first.first, y1 = points[a].first.second;
    double x2 = points[b].first.first, y2 = points[b].first.second;
...
int dfs(int u) {
    ...
    int cnt = 1;
    for (int v = 1; v <= n; v++) {
        if (!visited[v] && dis(u, v) <= points[u].second) {
            cnt += dfs(v);
        }
    }
    return cnt;
